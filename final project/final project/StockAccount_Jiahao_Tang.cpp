...
int StockAccount::buy(string fileName, string companySymbol, int shares, double buyPrice) {
	...
	if (shares * price <= cashBalance && price <= buyPrice) {
		...
		if (!portfolio->increaseShares(companySymbol, shares)) {
			...
			portfolio->addLast(newNode); 
			/*Using design pattern to allow user to select different sort methods.*/
			int sortChoice;
			/*Keep portfolio sorted.*/
			cout << "Which algorithm would you like to use to sort the stocks, Bubble sort or Select sort?" << endl;
			cout << "Enter 1 for bubble sort and enter 2 for select sort." << endl;
			cin >> sortChoice;
			if (sortChoice == 1) {
